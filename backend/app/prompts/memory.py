...
1. **第一層：適用性與八綱檢核 (Applicability Check - CRITICAL)**
   - **寒熱**: 參考案例是寒證，病人是否有熱象？
   - **虛實**: 參考案例是虛證，病人是否為實證？
   - **表裡**: 病位是否一致？
   - **判定**: 若八綱屬性出現 **根本性衝突**，請執行 **[拒絕機制]**：
     - 設定 `risk_flag: true`
     - 設定 `revised_diagnosis` 為 "CASE_REJECTED"
     - 停止後續修訂。
...
3. **第三層：階層式修訂 (Hierarchical Revision)**
   ...
   - **Step 3 定術**: 基於上述調整，增刪具體的治法與藥物策略。
     - **治則一致性檢核 (Treatment Consistency Check)**:
